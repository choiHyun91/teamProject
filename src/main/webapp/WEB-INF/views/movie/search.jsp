...
<head>
	...
	<style>
		...
		#cntResult{text-align:center; font-size: 1.5em;}
...
<body>
	...
	<div id="container">
		<div class="ui segment">
			...
			<br>
			<div id="cntResult">선택하신 조건에 해당하는 영화가 <span class="red bold">${paging.total }건</span> 검색되었습니다.</div>
	    </div>
		<br>
		<c:forEach var="movie" items="${search }" varStatus="idx">
			<div class="movieWrap">
			...
			<div class="center">
				<div class="ui pagination menu">
					<c:if test="${paging.startPage>paging.blockSize }">
						<a class="icon item" href="${conPath }/movie.do?method=search&pageNum=${paging.startPage-1}&movieName=${param.movieName}&movieGenre=${param.movieGenre}&movieCountry=${param.movieCountry}&minYear=${param.minYear}&maxYear=${param.maxYear}"><i class="left chevron icon"></i></a>
					</c:if>
					<c:forEach var="i" begin="${paging.startPage }"
						...
						<c:if test="${paging.currentPage!=i }">
							<a class="item" href="${conPath }/movie.do?method=search&pageNum=${i}&movieName=${param.movieName}&movieGenre=${param.movieGenre}&movieCountry=${param.movieCountry}&minYear=${param.minYear}&maxYear=${param.maxYear}">${i }</a>
						</c:if>
					</c:forEach>
					<c:if test="${paging.endPage<paging.pageCnt }">
						<a class="icon item" href="${conPath }/movie.do?method=search&pageNum=${paging.endPage+1}&movieName=${param.movieName}&movieGenre=${param.movieGenre}&movieCountry=${param.movieCountry}&minYear=${param.minYear}&maxYear=${param.maxYear}"><i class="right chevron icon"></i></a>
					</c:if>
				</div>
			</div>
		</c:if>
